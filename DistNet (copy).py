...
class DistNet(object):
    ...
    def log_results(self,log_file,res_dict,debug=False):
        for key,val in res_dict.iteritems():
            #print '%s: %.4f' % (key,val)
            if not debug: log_file.write(key + ' ' + str(val) + ' ')
        #print ''
        if not debug: log_file.write('\n')
        if not debug: log_file.flush()
    ...
    def __init__(self, params,outfile):
        ...
        print 'building dec...'
        x11,x11_log_std = self.VAEdecS([z1,s1])
        x11t,x11t_log_std = self.VAEdecS([z1,s1t])
        x12,x12_log_std = self.VAEdecS([z1,s2])
        xp2,xp2_log_std = self.VAEdecS([zn,s2])
...
#            return l1+l2
        ...
        self.DistNet.compile(optimizer='rmsprop', loss=['mse','mse',loss3,loss3,'binary_crossentropy','binary_crossentropy'],
        ...
        self.AdvNet.compile(optimizer='rmsprop', loss='binary_crossentropy')#loss#adv_loss)  
    ...
    def train(self,nb_epoch,X_train,X_valid,X_lbls):
        ...
        for e in range(nb_epoch):
            ...
            for (first, last) in zip(range(0, X_train.shape[0]-batch_size, batch_size),
                ...
                loss = loss0 + loss1
                mb = mb+1
            print 'train loss: '+str(round(loss,5)) +', G loss: ' + str(round(loss0,5)) + ', D loss: '+ str(round(loss1,5))
